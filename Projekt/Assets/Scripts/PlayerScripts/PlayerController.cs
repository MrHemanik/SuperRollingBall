...
namespace PlayerScripts
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            transform.position = GameObject.Find("SpawnpointTrigger").GetComponent<SpawnpointScript>().transform
                .position;
        ...
        private void FixedUpdate() //Updated 1-Mal pro Frame
        {
            ...
            if (_chargeJump)
            {
                ...
                {
                    UpdateCurrentJumpCharge(_currentJumpCharge +
                                            ((MAXJumpSpeed - MINJumpSpeed) / TimeTilMaxJump) * Time.fixedDeltaTime);
                }
                else
                {
                    UpdateCurrentJumpCharge(
                        MAXJumpSpeed); //Damit aus sowas wie 4,000001 eine 4 wird - im generellen aber redundant, da es keinen Unterschied macht.
                }
            }
            ...
            if (_movementVector.sqrMagnitude > 0.01)
            {
                ...
                forward.Normalize();
                //"Zeichne einen Pfeil von der Mitte in die Richtung die du willst und er berechnet den Winkel in den er sich bewegen soll" - Pascal 17.06.2021
                float angle =
                    (int) (Mathf.Atan2(_movementVector.x, _movementVector.y) * 180 /
                           Math.PI); //https://answers.unity.com/questions/914088/get-angle-of-vector2.html
        ...
        private void OnCollisionEnter(Collision other) //Bei Berührung eines Objektes (Kollision)
        {
            ...
            else if (other.gameObject.CompareTag("Spike")) //Alles was Damage macht hat "Spike" als Tag
            {
        ...
        {
            if (other.gameObject.CompareTag("PullRange"))
                return; //Falls die PullRange versucht das hier zu triggern wirds blockiert
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            if (other.gameObject.CompareTag("Water"))
            {
                ...
                statusObject.GetComponent<StatusEffectScript>()
                    .Initialize(2, 2, GetComponent<PlayerController>(), gameObject);
            }
        }
        ...
        private void BallRespawn(string s)
        {
            //Trigger der durch den Wiederbeleben Knopf getriggered wird.
        ...
        public void OnJump(InputAction.CallbackContext context)
        {
            ...
            {
                _jumpAllowed =
                    false; //Damit man sich nicht gegen eine Wand stellen kann, springt und dann noch einmal in der Luft springen kann
