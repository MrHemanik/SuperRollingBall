...
namespace PlayerScripts
{
    public class CameraController : MonoBehaviour
    {
        ...
        public Vector2
            zoomRange = new Vector2(2.0f,
                12.0f); //Vector, der angibt, dass von Distanz 2(x) bis 12(y) gezoomt sein kann.
        ...
        private Vector3
            _offsetRotation = new Vector3(0, 1, -1); // KameraOffset, eigentliche Kameraposition relativ zum Player
        ...
        private void Start()
        {
            ...
            transform.parent =
                null; //Detached sich, da es sonst zu komischen transforms kommt (bei Kugelbewegung kommt er mit der rotation nicht klar und wackelt trotz gegenrotation)
            GameManager.TriggerEvent(
                "FetchCurrentLevel"); //Aktiviert SetAnimation mit CurLevel & SetSkyboxColor mit Skybox[CurLevel]
        }

        private void
            FixedUpdate() //Eigentlich LateUpdate für Updates die was anzeigen sollen, da die als letztes berechnet werden, jedoch wird die Camera Transformed, was in Fixed gehört (sonst entsteht shutter)
        {
            if (!_cutScene)
            {
                //Position der Kamera
                ...
                if (!_fixedCamera)
                {
                    ...
                    //Ich habe viel durch StackOverflow und dem Unityforum gelernt, jedoch habe ich dort nur die Befehle wie AngleAxis oder Slerp gelernt und habe daraus die Kamerabewegung erstellt
                    ...
                    if (_cameraUpDownMovement[0] && y > 0)
                        y = 0; //Falls man nicht nach oben bewegen darf, der Input aber nach oben geht, wird der Input gelöscht
                    ...
                    if (_yRotation) //Falls Y-Rotation aktiviert ist
                    ...
                    _cameraUpDownMovement[0] =
                        (_offsetRotation.y >= MAXCameraHeight); //Muss false sein, bei true darf nicht bewegt werden
        ...
        {
            if (!movementValue.started)
                return; //soll nur beim "Start" trigger ausgelöst werden, sonst wird pro scroll 3-Mal gescrollt (start,perform,canceled)
