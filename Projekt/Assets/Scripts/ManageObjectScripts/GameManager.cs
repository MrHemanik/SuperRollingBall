...
namespace ManageObjectScripts
{
    ...
    */
        ...
        private static readonly string[] PermaUpgradeList = {"Live", "Live", "Hit", "Hit", "Hit", "Live", "Hit"};

        private static int
            _maxUnlockedLevel; //Speichert die Arraystelle aus _levelList für das höchstfreigeschaltende Level 

        private static int _maxLivePoints = 3;
        private static int _maxHitPoints = 3;

        private static bool[]
            _collectedPermaUpgrades = new bool[100]; //Index von permaUpgradeList, was für ein Upgrade es ist
        ...
        public static float
            GetHighscoreFromIndex(
                int i) //Ich musste es public machen, da sonst die LevelSelectionHighscores keinen guten Weg hätten, das abzufragen
        {
        ...
        {
            Debug.Log("GameManagerDeath wurde ausgeführt");
            _livePoints--;
            if (_livePoints <= 0)
            {
        ...
        {
            _coinCountInLevel =
                GameObject.Find("AllCoins").gameObject.transform
                    .childCount; //Muss nur beim Start eines neuen Levels geupdated werden
            TriggerEvent("UpdateLiveDisplay", _livePoints.ToString());
            TriggerEvent("UpdateCoinDisplay", _collectedCoinsInLevel + " / " + _coinCountInLevel);
        }
        ...
        private void
            UnlockNextLevel() //Testet, ob das geschaffte Level das höchst-freigeschaltende ist und agiert dementsprechend
        {
